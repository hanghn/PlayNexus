...
describe("socketSendMessage", () => {
  it("rejects invalid auth", async () => {
    ...
    await socketSendMessage(
      ...
      auth: badAuth,
      payload: { chatId: chat.chatId, text: "hi" },
    });
    expect(logSocketError).toHaveBeenCalledOnce();
  });

  it("stores the message and broadcasts chatNewMessage to the room", async () => {
    const chat = await createChat(new Date());
    await socketSendMessage(
      mockSocket,
      mockServer,
      ...
      auth,
      payload: { chatId: chat.chatId, text: "hello there" },
    });
    expect(logSocketError).not.toHaveBeenCalled();
...
describe("socketDeleteMessage", () => {
  it("removes the caller's own message and broadcasts chatMessageDeleted", async () => {
    const chat = await createChat(new Date());
    const record = (await getUserByUsername(auth.username))!;
    const message = await createMessage(record, "to delete", new Date());
    await addMessageToChat(chat.chatId, record, message.messageId);
    ...
    await socketDeleteMessage(
      ...
      auth,
      payload: { chatId: chat.chatId, messageId: message.messageId },
    });

    expect(logSocketError).not.toHaveBeenCalled();
    expect(mockServer.emit).toHaveBeenCalledWith("chatMessageDeleted", {
      chatId: chat.chatId,
      messageId: message.messageId,
    });
  });

  it("logs an error when deleting a message that isn't yours", async () => {
    const chat = await createChat(new Date());
    await socketDeleteMessage(
      mockSocket,
      mockServer,
      ...
      auth,
      payload: { chatId: chat.chatId, messageId: "no-such-message" },
    });
    expect(logSocketError).toHaveBeenCalledOnce();
  });
});
...
describe("socketLeave", () => {
  it("leaves the room and broadcasts chatUserLeft when in the room", async () => {
    const chat = await createChat(new Date());
    const leaveSocket = {
      id: "leaveSocket",
      rooms: new Set([chat.chatId]),
      leave: vi.fn(),
      emit: vi.fn(),
    ...
    await socketLeave(leaveSocket, mockServer)({ auth, payload: chat.chatId });

    expect(logSocketError).not.toHaveBeenCalled();
    expect(leaveSocket.leave).toHaveBeenCalledWith(chat.chatId);
    expect(leaveSocket.emit).toHaveBeenCalledWith(
      "chatUserLeft",
      expect.objectContaining({ chatId: chat.chatId }),
    );
  });

  it("logs an error when leaving a room it isn't in", async () => {
    const chat = await createChat(new Date());
    const leaveSocket = {
      id: "leaveSocket",
    ...
    expect(leaveSocket.leave).not.toHaveBeenCalled();
    expect(logSocketError).toHaveBeenCalledOnce();
  });
});
