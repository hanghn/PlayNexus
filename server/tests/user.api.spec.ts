...
const user1 = { username: "user1", display: "Yāo", bio: "", accentColor: "" };
const auth2 = { username: "user2", password: "pwd2222" };
const user2 = { username: "user2", display: "Sénior Dos", bio: "", accentColor: "" };
...
describe("POST/api/user/:username", () => {
  ...
  it("should persist accent color changes", async () => {
    // Use a fresh user so we don't depend on or disturb the shared fixtures
    const username = randomUUID().toString();
    const password = "pwd";
    response = await supertest(app).post("/api/user/signup").send({ username, password });
    expect(response.status).toBe(200);

    // Save an accent color
    response = await supertest(app)
      .post(`/api/user/${username}`)
      .send({ auth: { username, password }, payload: { accentColor: "#bfdbfe" } });
    expect(response.status).toBe(200);
    expect(response.body.accentColor).toBe("#bfdbfe");

    // The change should still be there on a fresh read
    response = await supertest(app).get(`/api/user/${username}`);
    expect(response.status).toBe(200);
    expect(response.body.accentColor).toBe("#bfdbfe");
  });
});
