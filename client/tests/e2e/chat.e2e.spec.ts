...
test.describe("Chat in the context of a Nim game", () => {
  ...
  test("avoids race conditions", async () => {
    // Typing 20 messages across two pages plus a reload is genuinely slow,
    // especially on a shared dev server, so give it the extended timeout.
    test.slow();
    // Chat starts collapsed, so open the panel on both pages before chatting.
    await page1.getByRole("button", { name: "Show chat panel" }).click();
    await page2.getByRole("button", { name: "Show chat panel" }).click();

    await page1.getByPlaceholder("Message").focus();
    await page2.getByPlaceholder("Message").focus();
    ...
    // The reloaded game page starts with chat collapsed again; reopen it.
    await page2.getByRole("button", { name: "Show chat panel" }).click();
