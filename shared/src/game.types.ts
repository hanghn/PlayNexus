...
import type { CribbageView } from "./games/cribbage.types.ts";
export * from "./games/cribbage.types.ts";
...
export const zGameKey = z.union([z.literal("nim"), z.literal("guess"), z.literal("cribbage")]);
 ...
 * - `type`: which game to create
 * - `singlePlayer`: if true, start immediately against an AI (cribbage only)
 * - `difficulty`: AI difficulty for single-player ("easy" default, or "hard")
 */
...
export const zCreateGamePayload = z.object({
  ...
  difficulty: z.union([z.literal("easy"), z.literal("hard")]).optional(),
});

export type TaggedGameView =
  | { type: "nim"; view: NimView }
  | { type: "guess"; view: GuessView }
  | { type: "cribbage"; view: CribbageView };
